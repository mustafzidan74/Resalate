...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['funeral_submit'])) {
    error_log("🚀 FORM SUBMITTED: funeral_submit exists");
    
    $current_user_id = get_current_user_id(); 
    
    $title = sanitize_text_field($_POST['text-title']);
    $content = wp_kses_post($_POST['funeral_content'] ?? '');
    $image_id = 0;
    $funeral_id = isset($_POST['funeral_id']) ? intval($_POST['funeral_id']) : 0;
    
    // Validate fields
    if (empty($title)) {
        error_log("❌ Title is missing.");
        wp_die('funeral title is required.');
    }
    ...
    if ($funeral_id && get_post_field('post_author', $funeral_id) == $current_user_id) {
        $post_args['ID'] = $funeral_id;
        $post_id = wp_update_post($post_args, true);
        error_log("✏️ Trying to update funeral ID = $funeral_id");
    } else {
        $post_id = wp_insert_post($post_args, true);
        error_log("🆕 Trying to insert new funeral");
    }
    
    if (is_wp_error($post_id)) {
        error_log("❌ Post insert/update failed: " . $post_id->get_error_message());
        wp_die('Error saving funeral: ' . $post_id->get_error_message());
    }
    
    error_log("✅ funeral saved successfully. ID = $post_id");
...
?>
      ...
      <main class="my-20">
        <div class="container mx-auto px-4 flex items-start gap-6">
          ...
          <div class="wrapper funerals flex-1">
            <div
              class="head-content flex justify-between items-center flex-wrap gap-4 mb-4"
            >
              <h2 class="sm:text-xl text-lg font-[900]">All funerals</h2>

              <!-- Upload Button -->
              <div class="new-funeral-btn mb-6">
                <button
                  class="primary-btn px-4 py-2 rounded transition-colors"
                  onclick="openfuneralModal()"
                >
                  Upload New funeral
                </button>
              </div>
            ...
            <div
              ...
              onclick="closefuneralModalByOverlay(event)"
            >
              <div
                ...
                id="funeralModalContent"
              >
                <!-- Close Button -->
                <button
                  onclick="closefuneralModal()"
                ...
                <form action="<?php echo esc_url(get_permalink()); ?>" method="post" enctype="multipart/form-data" id="new-funeral" class="flex flex-col gap-6">
                  <div class="input-wrapper py-4 px-2 rounded-lg">
                    <div class="box-wrapper space-y-6">
                      ...
                      <div>
                        ...
                        <!-- Toolbar -->
                        <div id="new-funerals-toolbar" class="mb-2">
                    ...
                    <input type="hidden" name="funeral_id" id="funeral_id" value="">
                    <input type="hidden" name="funeral_submit" value="1">
                    <button type="submit" name="funeral_submit" class="success-btn px-8 py-3 text-lg font-bold rounded w-fit self-end transition-colors mt-4 inline-block">Publish funeral</button>
                </form>
              </div>
            </div>
            ...
            >
              ...
              >
                <h2 class="text-xl font-bold mb-4">Delete funeral</h2>
                <p class="mb-6 text-gray-700">
                  Are you sure you want to delete this funeral? This action
                  cannot be undone.
                </p>
            ...
            <div class="mb-6">
              <input type="text" id="funeralSearch" placeholder="Find the funeral..." 
            ...
            >
            <?php
            $user_funerals = get_posts([
                'post_type' => 'funerals',
                'author' => $current_user_id,
                'posts_per_page' => -1,
            ]);
            
            foreach ($user_funerals as $funeral):
                $thumb = get_the_post_thumbnail_url($funeral->ID, 'medium') ?: 'https://placehold.co/600x400';
                $cat_slug = '';
                ?>
                <div class="card-box rounded-lg overflow-hidden shadow-md">
                    <div class="img-box">
                        <img src="<?php echo esc_url($thumb); ?>" alt="funeral Image" />
                    </div>
                    <div class="text-box px-4 py-5 bg-white flex flex-col">
                        <h3 class="title text-lg mb-2 font-[600]"><?php echo esc_html($funeral->post_title); ?></h3>
<p class="desc"><?php echo wp_trim_words(wp_strip_all_tags($funeral->post_content), 20); ?></p>
                        <div class="mt-6 grid grid-cols-2 gap-3">
                            <button
                              class="primary-btn py-2 px-6 rounded-lg"
onclick="editfuneral(<?php echo esc_js( wp_json_encode([
    'ID'      => $funeral->ID,
    'title'   => $funeral->post_title,
    'content' => $funeral->post_content,
    'image'   => $thumb
                            ...
                            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                                <?php wp_nonce_field('delete_funeral_' . $funeral->ID); ?>
                                <input type="hidden" name="action" value="delete_funeral">
                                <input type="hidden" name="funeral_id" value="<?php echo esc_attr($funeral->ID); ?>">
            ...
            <?php if (empty($user_funerals)): ?>
              <p class="text-gray-500 text-lg">No funerals found.</p>
            <?php endif; ?>
            
            </div>
            
            
            <p id="nofuneralsMsg" class="text-gray-500 text-lg hidden">No funerals found.</p>
...
const modalContent = document.getElementById("funeralModalContent");
...
function openfuneralModal() {
  ...
  // 🧹 تفريغ الحقول
  document.getElementById('funeral_id').value = '';
  document.getElementById('text-title').value = '';
  funeralImageImg.src = '';
  funeralImagePreview.classList.add('hidden');
  funeralImageInput.value = '';
  
  const quill = Quill.find(document.querySelector('#new-funerals-container'));
...
function closefuneralModalByOverlay(event) {
...
window.addEventListener("DOMContentLoaded", () => {
  quill = new Quill("#new-funerals-container", {
    theme: "snow",
    modules: {
      toolbar: "#new-funerals-toolbar",
    },
    placeholder: "Write your funeral content here...",
  });

  // Handle form submission
  document.getElementById('new-funeral').addEventListener('submit', function(e) {
    // Add Quill content to form
    const contentInput = document.createElement('input');
    contentInput.type = 'hidden';
    contentInput.name = 'funeral_content';
...
const funeralImageInput = document.getElementById("funeral-image");
const imageError = document.getElementById("imageError");
const funeralImagePreview = document.getElementById("funeralImagePreview");
const funeralImageImg = funeralImagePreview.querySelector("img");
const removefuneralImageBtn = document.getElementById("removefuneralImage");

funeralImageInput.addEventListener("change", function () {
  ...
  reader.onload = function (e) {
    funeralImageImg.src = e.target.result;
    funeralImagePreview.classList.remove("hidden");
  };
  reader.readAsDataURL(file);
});

removefuneralImageBtn.addEventListener("click", function () {
  funeralImageImg.src = "";
  funeralImagePreview.classList.add("hidden");
  funeralImageInput.value = "";
});
...
function editfuneral(data) {
  openfuneralModal();

  document.getElementById('text-title').value = data.title;
  document.getElementById('funeral_id').value = data.ID;
  ...
  if (data.image && data.image !== 'https://placehold.co/600x400') {
    funeralImageImg.src = data.image;
    funeralImagePreview.classList.remove('hidden');
  } else {
    funeralImageImg.src = '';
    funeralImagePreview.classList.add('hidden');
...
if (searchInput) {
  searchInput.addEventListener("input", function () {
    ...
    const nofuneralsMsg = document.getElementById("nofuneralsMsg");
    if (nofuneralsMsg) {
      nofuneralsMsg.style.display = visibleCount === 0 ? "block" : "none";
    }
