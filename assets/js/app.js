...
jQuery(document).ready(function($) {
  $('#sponsor-request-form').on('submit', function(e) {
    e.preventDefault();
    ...
    if ($('#phone').val() === '') {
      isValid = false;
      $('#phone').after('<p class="error-message">Please enter your phone number.</p>');
    }

    // Validate Message
    if ($('#message').val() === '') {
      isValid = false;
      $('#message').after('<p class="error-message">Please enter your message.</p>');
    }
    ...
    if (isValid) {
      ...
      $.ajax({
        url: ajax_object.ajaxurl, // Ensure this is defined in your localized JS object
        type: 'POST',
        data: {
          action: 'submit_sponsor_request',
          data: formData,
        },
        success: function(response) {
          if (response.success) {
            $('#form-message').html('<p>Thank you! Your request has been submitted.</p>');
            $('#sponsor-request-form')[0].reset(); // Reset form after successful submission
          } else {
            $('#form-message').html('<p>There was an error submitting your request. Please try again.</p>');
          }
        },
        error: function() {
          $('#form-message').html('<p>There was an error submitting your request. Please try again.</p>');
